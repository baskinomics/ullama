...
- **Decision:** Implement a layered defense strategy for context management:
  ...
  - **Server-Client Alignment:** Ensure `ctx-size` in `scripts/presets.ini` matches `limit.context` in `opencode.json` so the server rejects oversized payloads before allocation attempts
